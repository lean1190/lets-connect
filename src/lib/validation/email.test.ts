...
describe('isValidEmailDomain', () => {
  ...
  test.each(validEmailCases)('should return $expected for $description', ({ email, expected }) => {
    const result = isValidEmailDomain(email);
    expect(result).toBe(expected);
  });

  test.each(spamEmailCases)('should return $expected for $description', ({ email, expected }) => {
    const result = isValidEmailDomain(email);
    expect(result).toBe(expected);
  });

  test.each(caseInsensitiveCases)('should return $expected for $description', ({
    email,
    expected
  }) => {
    const result = isValidEmailDomain(email);
  ...
  test.each(invalidFormatCases)('should return $expected for $description', ({
    email,
    expected
  }) => {
    const result = isValidEmailDomain(email);
  ...
  test.each(edgeCases)('should return $expected for $description', ({ email, expected }) => {
    const result = isValidEmailDomain(email);
    expect(result).toBe(expected);
  });
});
